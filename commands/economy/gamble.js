...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (amount > balance || !balance || balance === 0)
      return message.channel.send("You don't have enough money.");

    // Optional:
    if (amount < 200)
      return message.channel.send(
        "You don't have enough money for gambling. The minimum was $200."
      );

    let cooldown = 10000; // 10 Detik.
    ...
    if (lastGamble !== null && cooldown - (Date.now() - lastGamble) > 0) {
      ...
      return message.channel.send(
        `Wooo that is too fast. You need to wait **${second}** second(s) before you can gambling again.`
      );
    }
    ...
    if (result < 5) {
      ...
      return message.channel.send(
        `Ahh, no. You lose $${amount}. Good luck next time.`
      );
    } else if (result > 5) {
      await db.set(`lastGamble.${message.author.id}`, Date.now());
      await db.add(`account.${message.author.id}.balance`, amount);
      return message.channel.send(
        `Woohoo! You won $${amount}! Good luck, have fun!`
      );
    }
  }
};
